...
#
# Prerequisites: Claude Code CLI must already be installed and
...
# ── Clean up the retired nano-banana skill ───────────────────
# Hero images are now generated by the API server via OpenAI (lib/images.ts);
# the Gemini CLI skill is no longer installed or used.

if [ -d "$HOME/.claude/skills/nano-banana" ]; then
  rm -rf "$HOME/.claude/skills/nano-banana"
  ok "Removed retired nano-banana skill from ~/.claude/skills"
fi
...
if [ -f "$PROJECT_ROOT/.env" ] && grep -q '""' "$PROJECT_ROOT/.env" 2>/dev/null; then
  ...
  printf "     • OpenAI (hero image generation — optional)\n"
